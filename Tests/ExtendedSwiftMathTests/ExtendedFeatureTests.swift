...
final class ExtendedFeatureTests: XCTestCase {
    ...
    func testGenfracSingleCharDelimitersRoundTrip() {
        // Single-character delimiters round-trip cleanly through \genfrac.
        for (l, r) in [("(", ")"), ("[", "]"), ("|", "|")] {
            let latex = "\\genfrac{\(l)}{\(r)}{0pt}{}{n}{k}"
            let list = build(latex)
            let frac = list.atoms[0] as! MTFraction
            XCTAssertEqual(frac.leftDelimiter, l)
            XCTAssertEqual(frac.rightDelimiter, r)
            XCTAssertEqual(MTMathListBuilder.mathListToString(list), latex)
            XCTAssertGreaterThan(render(latex).width, 0)
        }
    }

    func testGenfracMultiCharDelimiterRejected() {
        var error: NSError? = nil
        // Named/multi-character delimiter tokens (e.g. \langle) are out of scope and
        // are rejected rather than rendered as a wrong glyph.
        let named = MTMathListBuilder.build(fromString: "\\genfrac{\\langle}{\\rangle}{0pt}{}{n}{k}", error: &error)
        XCTAssertNotNil(error)
        XCTAssertNil(named)

        error = nil
        let multi = MTMathListBuilder.build(fromString: "\\genfrac{xyz}{)}{0pt}{}{n}{k}", error: &error)
        XCTAssertNotNil(error)
        XCTAssertNil(multi)
    }
