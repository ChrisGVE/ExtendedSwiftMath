...
**Philosophy**: This is a standalone, independently-published fork of `mgriebling/SwiftMath`. There are no upstream contributions planned. Changes should still blend with the existing codebase to keep the diff against upstream legible and maintenance manageable.
...
**Philosophy**: Each feature should be a self-contained, reviewable unit.
...
### 3. Release & Branch Process

**Philosophy**: All work targets this repository (`ChrisGVE/ExtendedSwiftMath`). There is no upstream PR pipeline.

**Implementation implications**:
- `main` is the single long-lived branch. Develop directly on `main`, or on short-lived feature branches merged back into `main`; do not keep a parallel `dev` branch.
- Commit messages and any PR/release bodies use plain markdown only - NO ANSI escape codes or terminal formatting, NO AI attribution.
- Releases are cut from `main` as bare `X.Y.Z` git tags plus a matching GitHub release (target `main`). Bug-fix = patch, new feature = minor.
- If a GitHub PR is ever opened, it targets `ChrisGVE/ExtendedSwiftMath` (our own repo). Use `--body-file` with a clean markdown file, never `--body` with heredoc/string, and verify with `gh pr view --json body`.

### 4. Package Naming Convention

**Philosophy**: Single, stable package name across the whole repository for independent publishing.

**Implementation implications**:
- `Package.swift` uses `name: "ExtendedSwiftMath"` on every branch (no per-branch rename).
- The library/product and module name remains `SwiftMath` for drop-in compatibility with the original.
...
ExtendedSwiftMath is a Swift implementation of LaTeX math rendering for iOS (11+) and macOS (12+). The package name is `ExtendedSwiftMath` on every branch; the product/module is always `SwiftMath` for drop-in compatibility with the original `mgriebling/SwiftMath`.
